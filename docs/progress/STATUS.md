...
The credential browser-state and safe replacement slices are complete. Continue under strict TDD with **capability-compatible model selection**. Reuse the existing Task 5 server-side model/capability resource, render only compatible choices for the selected provider/purpose, preserve persisted/server authority, and do not create a separate client-side model catalog or public provider call path.

After model selection, implement actionable accessible provider/capability error states. Do not advance to Task 10 until Task 9 has focused/full verification, exact-head green CI, and 0 unresolved Critical/Important correctness/security/accessibility/performance findings.

## Durable recovery

- `docs/milestones/M12-admin-onboarding-bots-providers.md` — authoritative M12 task/acceptance ledger.
- `docs/progress/M12-TASK9-CREDENTIAL-REPLACEMENT.md` — current Task 9 credential-replacement TDD/review/CI checkpoint.
