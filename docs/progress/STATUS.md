...
The public widget presents one completed Task 4 response progressively in one assistant container with a bounded 24 ms / 48-tick schedule, UTF-16-safe reveal boundaries, typing status, delayed copy/source controls, and stale-timer cancellation when the panel closes. The implementation deliberately does not invent public SSE/provider-specific streaming because M11's normalized streaming contract is application-layer authority and no reviewed public incremental transport seam currently projects it to the browser. Primary and cancellation/Unicode tests reached genuine behavioral RED at `0a018e848d6087291761aedac2e46037d86bc260` / CI `34729760794` and `9fb1b2c8e15f86630b85eff4abc14ca92a0ff644` / CI `34732327263`. Implementation/checkpoint heads `78cee1885406502e67810c1c2cacc9139e350e1b` / CI `34732421653` and `10a36bc469d50a117474600c03feff5c12b7f0fc` / CI `34734096420` are preserved as **NOT GREEN / NOT RED** because Prettier stopped before Jest. Final implementation GREEN is `82262b96c6088917d67e44922d007669c4e88065` / CI `34734183151`, all permanent jobs successful. Scoped fallback correctness/security/performance/accessibility/architecture review has **0 Critical / 0 Important unresolved**; independent reviewer transport remained unavailable. Evidence: `docs/progress/M14-TASK7-STREAMING-SIMULATED-TYPING.md`.

### Task 8 — administrator visual customizer/live preview — COMPLETE
The administrator bot screen provides seven bounded appearance controls, immediate non-network live preview using the same `WidgetAppearance`/`applyWidgetAppearance()` browser authority as the public runtime, and explicit persistence through the protected Task 3 appearance route. Bot-switching loads and saves use the shared `botAppearanceGeneration` latest-request-wins authority. Review-driven stale-save hardening preserves `a4a565586364adcd24213515e6420c990ee26e6d` / CI `34739706111` as **NOT RED** because Prettier stopped before Jest; genuine RED is `79e1609cd693661a80a919812dd86af4ecc0f1d9` / CI `34741381595`; genuine GREEN implementation is `362d47c65d4cafd43086e2862d606595341baa99` / CI `34741614657`. Subsequent verified Task 9 heads prove the Task 8 documentation state is no longer an unverified branch tip. Scoped fallback review has **0 Critical / 0 Important unresolved**; independent reviewer transport was unavailable. Evidence: `docs/progress/M14-TASK8-ADMIN-CUSTOMIZER.md`.

### Task 9 — block/direct/fullscreen embedding surfaces — COMPLETE
Floating, embedded, fullscreen, and dynamic Gutenberg adapters reuse one `PublicWidgetBootstrap` mount authority and one public widget runtime. The Gutenberg runtime/metadata are compiled and required in the package. Package/build TDD used genuine RED `3cf46e8f4f0b79472d26a1e5593d0e3c45aad1fe` / CI `34747519640` and GREEN `02962ed49882089548dffb813a75ade048bb9abb` / CI `34747631044`. Real WordPress surface smoke is GREEN at `2faf9677da3e8759021e22324b3d1f005f5a33de` / CI `34747870930`. Review-driven checkpoint `6ba3783f5b83d87790039d0774e0ad6f41d99231` / CI `34748005486` is explicitly **NOT RED**: the meaningful always-open behavior already passed and only an invalid test assumption about the floating-only `is-open` class failed. The repaired test-only head `a4150af0c29d46a23f24bbb9452079ca970c4224` / CI `34748133442` is GREEN across all four permanent jobs, including the real widget-surface smoke. Final fallback review has **0 Critical / 0 Important unresolved**. Evidence: `docs/progress/M14-TASK9-EMBED-SURFACES.md`.

## Current work

Proceed immediately to **Task 10 — milestone integration, real WordPress smoke, accessibility/mobile/performance/security review and closeout**. Preserve all existing M10/M11 production authorities, public REST boundaries, conditional asset behavior, and shared appearance/runtime contracts. Task 10 must verify and close the milestone; it must not create a parallel chatbot/RAG implementation.
...
- `docs/superpowers/plans/2026-09-13-m14-task9-embed-surfaces.md` — Task 9 adapter/build/smoke plan.
- `docs/progress/M14-TASK1-APPEARANCE-CONFIG.md` through `docs/progress/M14-TASK9-EMBED-SURFACES.md` — durable M14 task evidence.
