...
**Task 9 — Provider/model configuration screens** is the first legitimate unfinished task.

Begin Task 9 under strict TDD against the existing Task 4 credential resource and Task 5 model/readiness resources. The first behavioral RED should prove that an already-configured credential is represented only by configured/source or masked state and that no plaintext/ciphertext is rendered or rehydrated into the browser. Follow with credential replacement behavior, capability-filtered model selection, and actionable provider/capability errors.

Do not create a parallel credential path, do not persist completion solely in browser state, and do not advance to Task 10 until Task 9 has focused/full verification, exact-head green CI, and 0 unresolved Critical/Important correctness/security/accessibility/performance findings.
