...
The credential browser-state, safe replacement, and server-supplied selector-rendering slices are complete. Continue under strict TDD with **production Task 5 model-resource wiring**: request compatible generation models for the selected provider through the existing nonce-authenticated admin client, pass only the normalized server response into `ProviderSettingsScreen`, and clear/refetch model state when the selected provider changes. Do not create a separate client-side model catalog, client compatibility rules, or public provider call path.

Then implement actionable accessible provider/capability error states using the existing stable server error codes. Do not advance to Task 10 until Task 9 has focused/full verification, exact-head green CI, and 0 unresolved Critical/Important correctness/security/accessibility/performance findings.

## Durable recovery

- `docs/milestones/M12-admin-onboarding-bots-providers.md` — authoritative M12 task/acceptance ledger.
- `docs/progress/M12-TASK9-MODEL-SELECTOR.md` — current Task 9 model-selector TDD/review/CI checkpoint and exact continuation point.
