...
final class AdminRestBootstrap {
	...
	private static function appearance(): AppearanceRestResource {
		global $wpdb;
		$connection = new WpdbConnection( $wpdb );
		return new AppearanceRestResource( new WpdbBotAppearanceRepository( $connection, new TableNames( $connection->prefix() ) ) );
	}
	...
	private static function knowledge_detail(): KnowledgeDetailRestResource {
		...
		$connection = new WpdbConnection( $wpdb );
		$tables     = new TableNames( $connection->prefix() );
		return new KnowledgeDetailRestResource( new WpdbKnowledgeSourceRepository( $connection, $tables ), new WpdbDocumentRepository( $connection, $tables ), new WpdbChunkSearchStore( $connection, $tables ) );
	}
	...
	private static function knowledge_jobs(): KnowledgeJobRestResource {
		...
		$connection = new WpdbConnection( $wpdb );
		$tables     = new TableNames( $connection->prefix() );
		return new KnowledgeJobRestResource( new WpdbJobReadRepository( $connection, $tables ), new WpdbJobRepository( $connection, $tables ), new SystemClock() );
	}
