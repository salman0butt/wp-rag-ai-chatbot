...
final readonly class RetrievalFilter {
	...
	public function __construct(
		...
		foreach ( $source_ids as $source_id ) {
			if ( ! is_int( $source_id ) || $source_id < 1 ) {
				throw new InvalidArgumentException( 'Retrieval source IDs must be positive integers.' );
			}
		}
		...
		foreach ( $document_keys as $document_key ) {
			if ( ! is_string( $document_key ) || '' === trim( $document_key ) ) {
