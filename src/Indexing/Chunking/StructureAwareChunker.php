...
final class StructureAwareChunker {
	...
	public function chunks( DocumentRecord $document ): array {
		...
		$descriptors = array();
		foreach ( $this->structuredParagraphs( $content ) as $paragraph ) {
			foreach ( $this->splitToBudget( $paragraph['content'] ) as $piece ) {
				$descriptors[] = array(
					'content'      => $piece,
	 ...
	 */
	private function structuredParagraphs( string $content ): array {
		$lines = preg_split( '/\n/u', $content );
		if ( false === $lines ) {
	 ...
	 */
	private function splitToBudget( string $text ): array {
		...
		if ( count( $sentences ) < 2 ) {
			return $this->splitLexically( $text );
		}
		...
		foreach ( $sentences as $sentence ) {
			if ( $this->counter->count( $sentence ) > $this->config->maxTokens ) {
				...
				array_push( $result, ...$this->splitLexically( $sentence ) );
				continue;
			}
	 ...
	 * @return array<int, string>
	 */
	private function splitLexically( string $text ): array {
		...
		for ( $start = 0; $start < $count; $start += $this->config->maxTokens ) {
			...
			if ( $this->counter->count( $piece ) > $this->config->maxTokens ) {
				array_push( $result, ...$this->splitCodePointSafe( $piece ) );
				continue;
			}
			$result[] = $piece;
		}
	 ...
	 * @return array<int, string>
	 */
	private function splitCodePointSafe( string $text ): array {
