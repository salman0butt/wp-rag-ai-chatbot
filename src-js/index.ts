...
export const createAdminApiClient = (
  ...
  return {
    async request<T>(
      ...
      if (!response.ok) {
        throw new AdminApiError(response.status, getErrorCode(payload));
      }
...
const normalizeAdminHash = (hash: string): string => hash.replace(/^#\/?/, "");
...
const resolveSelectedKnowledgeSourceId = (hash: string): string | undefined => {
  const segments = resolveHashPath(hash).split("/");
  ...
  return createElement(
    "form",
    {
      ...
      onSubmit: (event: Event) => {
        ...
        const model = form.elements.namedItem("model_id") as HTMLInputElement;
        const fields = [name, provider, model];
        const validation = form.querySelector<HTMLElement>(`#${validationId}`);
...
export const KnowledgeManagementScreen = ({
  ...
  const rows = page.items.map((item) => {
    const linkProps: Record<string, unknown> = {
      href: `#/knowledge/${encodeURIComponent(item.id)}?page=${page.page}`,
    };
  ...
  const documentRows =
    documents?.items.map((item) => {
      ...
      return createElement(
        ...
        createElement("h3", null, createElement("a", linkProps, item.title)),
...
export const AdminShell = ({
    ...
    screenContent = createElement(
      ...
      botDisplayRules === undefined
        ...
        : DisplayRulesEditor({
            ...
            onChange: onChangeBotDisplayRules ?? (() => undefined),
  ...
  return createElement(
    ...
    createElement("nav", { "aria-label": "Administration" }, ...navigation),
...
export const bootstrapAdminApp = (hash = window.location.hash): boolean => {
  ...
  let cancelKnowledgeJob: (job: KnowledgeJobItem) => Promise<void> = async () =>
    undefined;
  let retryKnowledgeJob: (job: KnowledgeJobItem) => Promise<void> = async () =>
    undefined;
  ...
  let deleteBot: (bot: BotListItem) => Promise<void> = async () => undefined;
  let changeBotAppearance: (next: WidgetAppearance) => void = () => undefined;
  let saveBotAppearance: (next: WidgetAppearance) => void = () => undefined;
  let changeBotDisplayRules: (next: DisplayRulesConfig) => void = () =>
    undefined;
  let saveBotDisplayRules: (next: DisplayRulesConfig) => void = () => undefined;
  ...
  const renderState = (state: AdminShellState): void => {
    ...
    const selectedKnowledgeSourceId = resolveSelectedPersistedKnowledgeSourceId(
      currentHash(),
    );
    const selectedKnowledgeDocumentKey = resolveSelectedKnowledgeDocumentKey(
      currentHash(),
    );
    ...
    renderAdminShell(
      ...
      resolveAdminScreen(currentHash()) === "conversations" &&
        ...
        : undefined,
      providerId,
      providerId === loadedProviderId ? currentProviderCredential : undefined,
      providerId === loadedProviderModelsId ? currentProviderModels : undefined,
      providerId === loadedProviderModelsId ? currentProviderIssue : undefined,
      createBot,
      updateBot,
      deleteBot,
  ...
  const refreshBotAppearance = async (botId: string): Promise<boolean> => {
    ...
    try {
      ...
      currentBotAppearanceError = "Appearance settings could not be loaded.";
      loadedBotAppearanceId = botId;
      return true;
    }
  };
  const refreshBotDisplayRules = async (botId: string): Promise<boolean> => {
    ...
    try {
      ...
      currentBotDisplayRules = normalizeDisplayRules(response.display_rules);
  ...
  saveBotAppearance = (next: WidgetAppearance): void => {
    ...
    void client
      ...
      .catch(() => {
        if (isCurrentRequest()) {
          currentBotAppearanceError = "Appearance settings could not be saved.";
        }
      })
      .finally(() => {
        if (isCurrentRequest()) {
  ...
  saveBotDisplayRules = (next: DisplayRulesConfig): void => {
    ...
    void client
      ...
      .catch(() => {
        if (isCurrentRequest()) {
          currentBotDisplayRulesError = "Display rules could not be saved.";
        }
      })
      .finally(() => {
        if (isCurrentRequest()) {
  ...
  const refreshKnowledgeChunks = async (
    ...
    if (
      requestGeneration !== knowledgeSelectionGeneration ||
      resolveSelectedPersistedKnowledgeSourceId(currentHash()) !== sourceId ||
  ...
  const refreshKnowledgeSelection = async (
    ...
    try {
      ...
      const documentKey = resolveSelectedKnowledgeDocumentKey(currentHash());

      if (documentKey !== undefined) {
        return refreshKnowledgeChunks(sourceId, documentKey, requestGeneration);
      }
  ...
  const refreshProviderCredential = async (
    ...
    currentProviderCredential = {
      ...
      source:
        typeof credential.source === "string" ? credential.source : "none",
    };
  ...
  const refreshProviderSettings = async (providerId: string): Promise<void> => {
    currentProviderIssue = undefined;
  ...
  activeHashChangeHandler = () => {
      ...
      void refreshKnowledgePage(targetKnowledgePage)
        .then(async (isCurrentPage) => {
          ...
          if (selectedKnowledgeSourceId !== undefined) {
            await refreshKnowledgeSelection(selectedKnowledgeSourceId);
          }
    ...
    if (
      ...
      currentConversationDetail = undefined;
      loadedConversationDetailId = undefined;
      void refreshConversationDetail(conversationRoute.selectedConversationId)
        .then(() => renderState(stateFromReadiness()))
        .catch(() => renderState("error"));
      return;
    }
    ...
    if (
      screen === "providers" &&
      providerId !== undefined &&
      (providerId !== loadedProviderId || providerId !== loadedProviderModelsId)
    ) {
